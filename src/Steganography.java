...
public class Steganography
{
	 ...
	 */
	public boolean encode(String path, String original, String ext1, String stegan, String message,int old_key)
	{
		...
		BufferedImage image = add_text(image_old,message,key);
	...
	public String decode(String path, String name,int key)
	{
		byte[] decode;
		try
		{
			...
			BufferedImage image  = user_space(getImage(image_path(path,name,"png")));
			decode = decode_text(get_byte_data(image),key);
	 ...
	 */
	private BufferedImage add_text(BufferedImage image, String text,int key)
	{
		...
		try
		{
			encode_text(img, bkey, 0,key); // 64 bytes for key using LSB1 and LSB2
			encode_text(img, len,  32,key); //16 bytes for length using LSB1 and LSB2
			encode_text(img, msg, 64 ,key); //remaining bytes for message
		}
		catch(Exception e)
		{
			JOptionPane.showMessageDialog(null, 
"Target File cannot hold message!", "Error",JOptionPane.ERROR_MESSAGE);
	...
	{
		//int key_real=0;//Doesn't really required if user provides the actual keys(for checking only)
		int length = 0;
